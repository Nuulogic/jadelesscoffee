...
jadelesscoffee.Compiler.prototype = {
    ...
    buildCoffeeFile: function(filename, callback) {
        if (filename) {
            ...
            var relativePathToSourceFile = path.relative(path.dirname(pathToSourceMapFile), path.dirname(filename))
            console.log(relativePathToSourceFile)
            ...
            try {
                var coffeeJs = coffee.compile(data, {
                    ...
                    sourceRoot: relativePathToSourceFile + '/'
                });
            }
            catch(err) {
                if (callback)
                ...
                this.compileFile(filename, ScopeLocker.lock(this, function(error, filename) {
                    if (error == null)
                        ...
                        if (this.options.html) {
                            ...
                            var fileType = path.extname(filename).substr(1);
                            console.error(this.generatePythonErrorMessage(error, filename, fileType));
                        }
                        else
                            console.error(error);
                }));
    ...
    /**
     * Returns dict with information on where to find the error for use in Python
     *
     * Since every compiler returns a different style of errorObject, type is used to determine how to parse the errorObject.
     */
    generatePythonErrorMessage: function(errorObject, filename, type) {
        ...
        if (type == 'coffee') {
            ...
            if (errorObject.location) {
                lineNumber = errorObject.location.first_line;
                error.lineNumber = errorObject.location.first_line;
            }
            else if (lineNumber && lineNumber.length > 0) {
                lineNumber = Number(lineNumber[1]);
            ...
            error.message = actualErrorMessage;
            //parse the errorObject string
            var lineNumber = errorMessage.match(/(?:.*?)Error: (?:.+?):(\d+)(?:.*)/m);
            var startLineNumber = 0;
            var endLineNumber = 6;
            if (lineNumber && lineNumber.length > 0) {
                lineNumber = Number(lineNumber[1]);
